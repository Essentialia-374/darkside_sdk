...
#include "../utils/no_spread.hpp"

struct lag_record_t {
	vec3_t m_origin{};
	c_cs_player_pawn* m_pawn{};
	c_skeleton_instace* m_skeleton = nullptr;
	c_bone_data m_bone_data[256];
	c_bone_data m_bone_data_backup[256];
	...
	inline void store(c_cs_player_pawn* pawn) {
		...
		setup_bones(m_skeleton, 256);

		std::memcpy(m_bone_data, skeleton_instance->m_bone_cache, sizeof(matrix2x4_t) * min(skeleton_instance->m_bone_count, 256));
	...
	inline void apply(c_cs_player_pawn* pawn) {
		...
		std::memcpy(m_bone_data_backup, skeleton_instance->m_bone_cache, sizeof(matrix2x4_t) * min(skeleton_instance->m_bone_count, 256));

		std::memcpy(skeleton_instance->m_bone_cache, m_bone_data, sizeof(matrix2x4_t) * min(skeleton_instance->m_bone_count, 256));
	}
	...
	inline void reset(c_cs_player_pawn* pawn) {
		...
		std::memcpy(skeleton_instance->m_bone_cache, m_bone_data_backup, sizeof(matrix2x4_t) * min(skeleton_instance->m_bone_count, 256));
	}
...
class c_rage_bot {
	...
	std::unordered_map<int, util::circular_buffer<lag_record_t>> m_lag_records{};

	std::unordered_map<int, aim_target_t> m_aim_targets{};
	aim_target_t* m_best_target{};
	int m_last_hit_chance = 0;

	inline const char* GetExtractedWeaponName(c_base_player_weapon* weapon)
	{
		if (!weapon)
			return "";
		...
		const char* szWeaponName = weapon_data->m_name();
