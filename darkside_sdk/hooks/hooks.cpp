...
bool c_hooks::initialize() {
	// No need for MH_Initialize with PolyHook 2

	g_skins->dump_items();

	// Virtual function hooks using PolyHook 2's VFuncSwapHook
	create_move::m_create_move.hook(g_interfaces->m_csgo_input, {
		{5, (uint64_t)create_move::hk_create_move}
		});

	enable_cursor::m_enable_cursor.hook(g_interfaces->m_input_system, {
		{76, (uint64_t)enable_cursor::hk_enable_cursor}
		});

	validate_view_angles::m_validate_view_angles.hook(g_interfaces->m_csgo_input, {
		{7, (uint64_t)validate_view_angles::hk_validate_view_angles}
		});

	mouse_input_enabled::m_mouse_input_enabled.hook(g_interfaces->m_csgo_input, {
		{19, (uint64_t)mouse_input_enabled::hk_mouse_input_enabled}
		});
...
void c_hooks::destroy() {
	...
	// No need for MH_Uninitialize with PolyHook 2
}

void __fastcall hooks::create_move::hk_create_move(i_csgo_input* rcx, int slot, bool active) {
	static auto original = m_create_move.get_original< decltype(&hk_create_move) >(5);
...
bool hooks::mouse_input_enabled::hk_mouse_input_enabled(void* ptr) {
	static auto original = m_mouse_input_enabled.get_original< decltype(&hk_mouse_input_enabled) >(19);
	return g_menu->m_opened ? false : original(ptr);
}

void* hooks::enable_cursor::hk_enable_cursor(void* rcx, bool active) {
	static auto original = m_enable_cursor.get_original< decltype(&hk_enable_cursor) >(76);
...
void hooks::validate_view_angles::hk_validate_view_angles(i_csgo_input* input, void* a2) {
	static auto original = m_validate_view_angles.get_original< decltype(&hk_validate_view_angles) >(7);
